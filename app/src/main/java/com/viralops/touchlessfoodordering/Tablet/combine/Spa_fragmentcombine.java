...
public class Spa_fragmentcombine extends Fragment {
    private Ringtone r;
    ...
    public static Spa_fragmentcombine newInstance() {
        return new Spa_fragmentcombine();
    }
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
        ...
        try {
            MainActivity.newordersspa.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    if (queuelist.size() != 0) {
                        try {
                            if(MainActivity.bellspa.getVisibility()==v.VISIBLE) {
        ...
        shimmerRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                if(MainActivity.bellspa.getVisibility()==View.GONE) {
        ...
        (RetrofitClientInstance.getApiService().Spa_getHeader("Bearer "+sessionManager.getACCESSTOKEN())).enqueue(new Callback<Header>() {
            ...
            public void onResponse(@NonNull Call<Header> call, @NonNull Response<Header> response) {
                ...
                if(response.code()==202||response.code()==200){
                    ...
                    MainActivity.newtotalspa.setText(String.valueOf(login.getData().getNew_order()));
                    MainActivity.totalorderspa.setText(String.valueOf(login.getData().getNew_order()+login.getData().getAccepted_order()+login.getData().getCleared_order()));
                    MainActivity.inprogresstotalspa.setText(String.valueOf(login.getData().getAccepted_order()));
